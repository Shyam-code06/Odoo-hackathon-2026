...
const getDrivers = async (req, res, next) => {
  try {
    const drivers = await driverService.getDrivers(req.query, getFMId(req));
...
const getAvailableDrivers = async (req, res, next) => {
  try {
    const drivers = await driverService.getAvailableDrivers(getFMId(req));
    return res.status(200).json(drivers);
  } catch (err) { next(err); }
};
...
const getLicenseExpiringDrivers = async (req, res, next) => {
  try {
    const drivers = await driverService.getLicenseExpiringDrivers(getFMId(req));
    return res.status(200).json(drivers);
  } catch (err) { next(err); }
};
...
const getDriverById = async (req, res, next) => {
  try {
    const driver = await driverService.getDriverById(req.params.id, getFMId(req));
...
const createDriver = async (req, res, next) => {
  try {
    ...
    const driver = await driverService.createDriver(req.body, getFMId(req));
...
const updateDriver = async (req, res, next) => {
  try {
    const driver = await driverService.updateDriver(req.params.id, req.body, getFMId(req));
    return res.status(200).json(driver);
  } catch (err) {
    if (err.message.includes('not found')) return res.status(404).json({ error: err.message });
...
const deleteDriver = async (req, res, next) => {
  try {
    await driverService.deleteDriver(req.params.id, getFMId(req));
...
const exportCSV = async (req, res, next) => {
  try {
    ...
    const rows = drivers.map(d => [
      ...
      d.contactNumber, d.safetyScore, d.status
    ]);
...
module.exports = { getDrivers, getAvailableDrivers, getLicenseExpiringDrivers, getDriverById, createDriver, updateDriver, deleteDriver, exportCSV };
