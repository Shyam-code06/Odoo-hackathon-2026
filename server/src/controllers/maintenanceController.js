...
const getMaintenanceLogs = async (req, res, next) => {
  try {
    const logs = await maintenanceService.getMaintenanceLogs(req.query, getFMId(req));
...
const getMaintenanceLogById = async (req, res, next) => {
  try {
    const log = await maintenanceService.getMaintenanceLogById(req.params.id, getFMId(req));
...
const createMaintenanceLog = async (req, res, next) => {
  try {
    ...
    const log = await maintenanceService.createMaintenanceLog(req.body, getFMId(req));
    return res.status(201).json(log);
  } catch (err) {
    if (err.message.includes('not found')) return res.status(404).json({ error: err.message });
    if (err.message.includes('active trip') || err.message.includes('retired')) return res.status(400).json({ error: err.message });
    next(err);
  }
};
...
const completeMaintenanceLog = async (req, res, next) => {
  try {
    const log = await maintenanceService.completeMaintenanceLog(req.params.id, req.body, getFMId(req));
    return res.status(200).json(log);
  } catch (err) {
    if (err.message.includes('not found')) return res.status(404).json({ error: err.message });
    if (err.message.includes('already closed')) return res.status(400).json({ error: err.message });
    next(err);
  }
};
...
const deleteMaintenanceLog = async (req, res, next) => {
  try {
    await maintenanceService.deleteMaintenanceLog(req.params.id, getFMId(req));
...
module.exports = { getMaintenanceLogs, getMaintenanceLogById, createMaintenanceLog, completeMaintenanceLog, deleteMaintenanceLog, exportCSV };
